...
- 成功单步的 vLLM server 使用 `max_num_seqs=2`，而不是 KV token 上限。launcher 已改为默认 `train_batch_size * ROLLOUT_N`，并在 16-row fixture 上确认该值传递为 `2`；V1 初始化仍未完成，因此这个配置修复也不构成 RL 改善证据。
