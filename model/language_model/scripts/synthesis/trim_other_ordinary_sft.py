"""Trim only unwanted labels inside the ordinary SFT ``other_noncoding`` bucket."""
...
KEEP = {
    "math_statistics", "natural_science", "language_summary_text",
    "software_it_digital", "history_religion_philosophy", "general_unclear",
}
EXPECTED = {
    "train": {"input": 122305, "other": 68761, "other_kept": 27975, "removed": 40786},
    "test": {"input": 15110, "other": 7605, "other_kept": 3110, "removed": 4495},
}
PATTERNS = {
...
def other_label(prompt: str) -> str:
...
def keep_label(prompt: str) -> str:
    # Preserve coding and every row outside the old catch-all bucket.
    if is_coding(prompt) or topic(prompt) != "other_noncoding":
        return "outside_other_keep"
    return other_label(prompt)
...
def process(path: Path, target: Path | None = None) -> dict:
    ...
    counts, kept_labels = Counter(), Counter()
    try:
        for batch in parquet.iter_batches(batch_size=2048):
            labels = [keep_label(value or "") for value in batch.column("prompt").to_pylist()]
            counts.update(labels)
            mask = [label == "outside_other_keep" or label in KEEP for label in labels]
            kept_labels.update(label for label, retain in zip(labels, mask) if retain)
            if writer and any(mask):
                writer.write_batch(batch.filter(pa.array(mask)))
    finally:
        if writer:
            writer.close()
    other = sum(value for label, value in counts.items() if label != "outside_other_keep")
    other_kept = sum(value for label, value in kept_labels.items() if label != "outside_other_keep")
    return {"input": sum(counts.values()), "other": other, "other_kept": other_kept, "removed": other - other_kept, "labels": dict(counts)}
...
def main() -> None:
    ...
    report = {"policy": "trim_other_noncoding_keep_six_v1", "keep_inside_other": sorted(KEEP), "splits": {}}
    if not args.dry_run:
        archive = args.root / "archive/ordinary_other_trim_20260825"
        temporary = args.root / ".ordinary_other_trim_tmp"
        report_path = args.root / "ordinary_other_trim_20260825.json"
        if archive.exists() or temporary.exists() or report_path.exists():
            raise RuntimeError("refusing to reuse output paths")
        temporary.mkdir()
    else:
        archive = temporary = report_path = None
    try:
        for split, split_paths in paths.items():
            files = {}
            for source in split_paths:
                target = temporary / source.name if temporary else None
                files[source.name] = process(source, target)
            totals = {key: sum(item[key] for item in files.values()) for key in ("input", "other", "other_kept", "removed")}
            if totals != EXPECTED[split]:
                raise RuntimeError(f"{split} safety mismatch: {totals} != {EXPECTED[split]}")
