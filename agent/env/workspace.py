"""Small, dependency-free workspace state snapshots for agent episodes."""
...
def snapshot(root: str | Path) -> dict[str, Any]:
    """Return a stable, JSON-serializable file-state snapshot.

    File contents are represented by hashes so large observations do not get
    copied into every event. Symlinks are skipped: an episode must not acquire
    state outside its workspace through a link.
    """
