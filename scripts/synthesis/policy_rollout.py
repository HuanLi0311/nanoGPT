...
def openai_client(base_url: str, model: str, api_key: str = "", timeout: int = 120,
    ...
    def complete(messages: list[dict[str, Any]], tools: list[dict[str, Any]]) -> dict[str, Any]:
        body = json.dumps({"model": model, "messages": messages, "tools": tools,
                           "tool_choice": "auto", "temperature": temperature}).encode()
...
def _schemas(names: list[str]) -> tuple[dict[str, Any], dict[str, Any]]:
...
_schemas.workspace_root = Path("/tmp/nanoagent-policy-rollouts")
...
async def _one(task: dict[str, Any], complete: Complete, policy_kind: str, model: str,
               index: int, max_turns: int) -> dict[str, Any]:
    tools, schemas = _schemas(task["available_tools"])
    ...
    try:
        for turn in range(max_turns):
            ...
            if calls:
                ...
                messages[-len(normalized) - 0:-len(normalized)] = [assistant]
            else:
...
def run_policy_tasks(tasks_path: Path, output: Path, *, complete: Complete, policy_kind: str,
    ...
    root = workspace_root or output.parent / "policy_workspaces"
    _schemas.workspace_root = root
    rows = []
    for index, task in enumerate(tasks):
        try:
            rows.append(asyncio.run(_one(task, complete, policy_kind, model, index, max_turns)))
            ...
            rows.append({"task_id": task.get("task_id"), "policy": {"kind": policy_kind, "model": model},
                         ...
                         "harness_status": "transport_fault", "eligible": False, "reason": str(error)}})
...
def main() -> None:
    ...
    parser.add_argument("tasks", type=Path); parser.add_argument("output", type=Path)
    ...
    parser.add_argument("--api-key-env", default="OPENAI_API_KEY"); parser.add_argument("--timeout", type=int, default=120)
    parser.add_argument("--temperature", type=float, default=0.0); parser.add_argument("--max-turns", type=int, default=20)
    parser.add_argument("--limit", type=int); parser.add_argument("--workspace-root", type=Path)
    parser.add_argument("--keep-workspaces", action="store_true")
    args = parser.parse_args()
    client = openai_client(args.base_url, args.model, os.environ.get(args.api_key_env, ""), args.timeout, args.temperature)
